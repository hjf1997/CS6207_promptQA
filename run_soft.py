...
def run_prompt(args, logger):
    ...
    if args.do_train:
        ...
        if args.checkpoint is not None:
            model = QABart_prompt.from_pretrained("facebook/bart-base", args, state_dict=torch.load(args.checkpoint))
        else:
            model = QABart_prompt.from_pretrained("facebook/bart-base", args)
    ...
    if args.do_predict:
        ...
        model = QABart_prompt.from_pretrained("facebook/bart-base", args, state_dict=torch.load(checkpoint))
        # if args.randomize_prompt:
        #     model.randomize_prompt()
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for batch in t:
            ...
            loss = model(input_ids=batch[0], attention_mask=batch[1],
                         decoder_input_ids=batch[2], decoder_attention_mask=batch[3], q2_input_ids=batch[4],
                         q2_attention_mask=batch[5], is_training=True)
            if len(args.gpu_ids) > 1:
...
def inference(model, dev_data, save_predictions=False):
    ...
    for i, batch in enumerate(dev_data.dataloader):
        ...
        input_embeds, attention_mask = model.get_input_embeds(input_ids=batch[0], attention_mask=batch[1], q2_input_ids=batch[2], q2_attention_mask=batch[3], random=model.random_init)
